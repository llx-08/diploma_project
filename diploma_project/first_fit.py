...
def first_fit(network_service, length, env):
    ...
    for i in range(length):
        ...
        for cpu in list_cpus:
            if len(placement) < i:
                return None, 0, 0, 0, 0
            if placement != [] and placement[i - 1] != current_cpu:
                change_cpu = True
            else:
                change_cpu = False

            # 检查当前检查的主机和链路是否有足够空间放置VNF，如果当前放置的VNF放置在了两个不同的主机上则需要考虑链路是否满足条件（change_cpu）
            if size <= (env.cpu_properties[cpu]["numSlots"] - env.cpu_used[cpu]) and (
            2 * env.bandwidth <= (env.link_properties[cpu]["bandwidth"] - env.link_used[cpu]) if change_cpu else True):
